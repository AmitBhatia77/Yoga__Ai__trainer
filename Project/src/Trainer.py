...
class Trainer():
    ...
    def encode_data(self,classes, df):
        nClass = len(classes)
        ...
        for idx, className in enumerate(classes):
            ...
            for start in range(nSample-self.N_TIME):
                X.append(df[className].iloc[start:start+self.N_TIME,:])
                one_hot = [0]*nClass
                one_hot[idx] = 1
                y.append(one_hot)
    ...
    def get_model(self,input_shape, classes):
        model = Sequential([
    ...
    def output_loss(self,hist, path):
        fig, ax = plt.subplots(1, 2, figsize=(20,8))
        epochs = range(1,self.N_EPOCH+1)
    ...
    def training(self):
        classes, df = self.load_data()
        X, y = self.encode_data(classes, df)
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.2)
        input_shape = (X_train.shape[1], X_train.shape[2])
        model = self.get_model(input_shape, classes)
        ...
        self.output_loss(hist, '../img/result.png')
        self.result = None
        self.run_event.clear()
        return None
